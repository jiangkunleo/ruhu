...
class News extends Common{
	...
	public function index( ) {
		...
		//分页
		$list  = $newsModel->order('id','asc')->paginate(5); //分页，每页5条数据
	...
	public function add(Request $request ) {
		...
		if( $request->isPost() ) {
			...
			$data['seo_title'] = htmlspecialchars(input('post.seo_title'));
			$data['seo_keyword'] = htmlspecialchars(input('post.seo_keyword'));
			$data['seo_description'] = htmlspecialchars(input('post.seo_description'));
			$data['add_time']  =  date('Y-m-d H:i:s',time());
			$data['is_valid']  =  input('post.is_valid');
			$data['is_prior']  =  input('post.is_prior');
			$data['is_notice'] =  input('post.is_notice');
			//**验证数据
			$rules = [
		    ...
		    if( $info!== false ){  
		    	...
		    	$res = $image->thumb(150,150,\think\Image::THUMB_CENTER)->save($thumb_logo_path );
		    	...
		    	if($res!==false) {
				    ...
				    if($status !== false) {
				    	...
				    	$this->success('成功添加一条新闻！',url('admin/news/index'));die;
				    }else{
				    	$this->error('添加失败！');die;
				    }	
				...
				if($info!==false) {
			    	...
			    	if($res!==false) {			    		
						...
						if($status !== false) {
					    	...
					    	$this->success('更改新闻成功！',url('admin/news/index'));die;
					    }else{
	...
	public function del(Request $request) {
		...
		if($request->isGet()) {
			...
			$info 			  = NewsModel::get($current_new_id); //根据id获取当前新闻信息
			...
			if($status!==false) {
			    ...
			    $this->success('新闻删除成功！','admin/news/index',1);die;
			}else{
				$this->error('删除失败!');die;
			}
		}
	...
	public function notice(Request $request) {
		$newsModel      = new NewsModel();
		$current_new_id = input('id');
		$new_data_obj   = $newsModel->where('id',$current_new_id)->field('is_notice')->select();
		$new_data = $new_data_obj->toArray();
		if($new_data[0]['is_notice']=='Y') {
			$newsModel->where('id',$current_new_id)->update(['is_notice'=>'N']);
		}else{
			$newsModel->where('id',$current_new_id)->update(['is_notice'=>'Y']);	
		}
	...
	public function prior(Request $request) {
		$newsModel      = new NewsModel();
		$current_new_id = input('id');
		$new_data_obj   = $newsModel->where('id',$current_new_id)->field('is_prior')->select();
		$new_data = $new_data_obj->toArray();
		if($new_data[0]['is_prior']=='Y') {
			$newsModel->where('id',$current_new_id)->update(['is_prior'=>'N']);
		}else{
			$newsModel->where('id',$current_new_id)->update(['is_prior'=>'Y']);	
		}
		$this->redirect('/admin/news/index');die;
	}	
}
