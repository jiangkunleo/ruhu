...
use app\mobil\model\Test as TestModel;
use app\mobil\model\Order as OrderModel;
use app\mobil\model\About as AboutModel;
use app\mobil\model\Examples as ExamplesModel;
/**
 *  手机前台首页
 */
class Index extends Controller{
    ...
    public function index() {
        ...
        $banner_data = $bannerModel->where(['is_valid'=>'Y','type'=>0])->select();
        $this->assign('banner_data',$banner_data);

        //获取入户政策信息
        $examplesModel = new ExamplesModel();
        $cases_list  = $examplesModel->order('id','desc')->select();       
    ...
    public function test(Request $request) {
        if( $request->isPost() ) {
            ...
            if( $status !== false ) {
                $this->success('测评信息上传成功，我们工作人员将会与您联系！',url('mobil/index/index'));die;
            }else{
